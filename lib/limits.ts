...
export function limitProblem(format: Format, mode: PlayMode, units: number): string | null {
  ...
  if (units < min) {
    ...
    return `Add ${need} more ${need === 1 ? noun.slice(0, -1) : noun} — ${formatName(format)} ${mode === 'teams' ? 'teams' : 'singles'} needs at least ${min}.`;
  }
  if (units > max) {
...
export function formatName(format: Format): string {
  return format === 'americano' ? 'Americano' : 'Mexicano';
}
